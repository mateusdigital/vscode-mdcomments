...
function _CreateCommentInfo(editor: vscode.TextEditor)
{
  ...
  let single_start = (comment_info.lineComment) ? comment_info.lineComment : comment_info.blockComment[0];
  let single_end   = (comment_info.lineComment) ? ""                       : comment_info.blockComment[1];
  let multi_start  = (comment_info.lineComment) ? comment_info.lineComment : comment_info.blockComment[0];
  let multi_end    = (comment_info.lineComment) ? comment_info.lineComment : comment_info.blockComment[1];
  let multi_middle = (comment_info.lineComment) ? comment_info.lineComment : comment_info.blockComment[0][comment_info.blockComment[0].length-1];
  ...
  selectionColumn: number)
{
    ...
    const max_columns = 80 - (selectionColumn + info.singleLineLength + space_start.length + space_end.length);
    const spacer      =  "-".repeat(max_columns);
    const comment_line = `${info.singleLineStart}${space_start}${spacer}${space_end}${info.singleLineEnd}`;
...
{
  // ---------------------------------------------------------------------------
  const single_line_disposable = vscode.commands.registerCommand("mdcomments.singleLineComment", ()=>{
    try {
  ...
  // ---------------------------------------------------------------------------
  const multi_line_disposable = vscode.commands.registerCommand("mdcomments.multiLineComment", ()=>{
    try {
      const curr_editor = vscode.window.activeTextEditor;
      if(!curr_editor) {
        return;
